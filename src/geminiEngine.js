// ══════════════════════════════════════════════════════════
//   GEMINI GHOST-CANDLE PREDICTOR
//   Asks Gemini to predict the NEXT 1-minute candle's exact
...
OPEN|HIGH|LOW|CLOSE|CONFIDENCE|REASON
OPEN, HIGH, LOW, CLOSE are decimal numbers for the predicted next 1-minute candle, using the same number of decimal places as the input prices. OPEN must equal the last known close price given to you. HIGH must be >= max(OPEN, CLOSE). LOW must be <= min(OPEN, CLOSE).
...
Never refuse to answer — always output a definite predicted candle based on the data given.`

/**
 * Predicts the next candle's O/H/L/C via Gemini.
 * @param {string} key - Gemini API key
 * @param {string} market - display name e.g. "GBP/USD"
 * @param {Array} candles - chronological OHLC candle history (objects with open/high/low/close as strings or numbers)
 * @param {object} engineResult - the object returned by runSignalEngine (direction, strength, breakdown, confidence)
 * @returns {Promise<{open:number, high:number, low:number, close:number, confidence:number, reason:string, ok:boolean}>}
 */
export async function predictNextCandle(key, market, candles, engineResult) {
  ...
  // Sane fallback if Gemini fails or the app has no key: project a small
  // move in the engine's own direction, sized off recent candle ranges —
  // never leaves the ghost candle empty.
  const fallback = (reasonText) => {
    ...
    const close = bullish ? lastClose + move : lastClose - move
    const high = Math.max(lastClose, close) + avgRange * 0.15
    const low = Math.min(lastClose, close) - avgRange * 0.15
    return {
      open: Number(lastClose.toFixed(decimals)),
...
Last known close (this is the OPEN of the candle you predict): ${lastClose.toFixed(decimals)}
...
Respond with exactly one line: OPEN|HIGH|LOW|CLOSE|CONFIDENCE|REASON`
    ...
    const res = await fetch(`${GEMINI_URL}?key=${encodeURIComponent(key)}`, {
      ...
      body: JSON.stringify({
        ...
        generationConfig: { temperature: 0.15, maxOutputTokens: 120, candidateCount: 1 },
      }),
    })
    ...
    if (parts.length >= 6) {
      ...
      const confidence = parseInt(parts[4].replace(/[^0-9]/g, ''), 10)
      const reason = parts.slice(5).join('|').trim()

      const numsValid = [open, high, low, close].every(n => Number.isFinite(n))
      const rangeValid = numsValid && high >= Math.max(open, close) && low <= Math.min(open, close)

      if (numsValid && rangeValid) {
        return {
